...
class Discount(models.Model):
    ...
    necessity_only = models.BooleanField(
        ...
        help_text=_(
            "Is this discount for basic necessities or prime commodities only?"),
        default=False
    )
...
class SalesReport(models.Manager):
    def generate_report(self, sales, cashier):
        ...
        xreading.first_si = SalesInvoice.objects.filter(
            sales__in=sales).aggregate(val=Min('id'))['val'] or 0
        xreading.last_si = SalesInvoice.objects.filter(
            sales__in=sales).aggregate(val=Max('id'))['val'] or 0
...
class Sales(models.Model):
    ...
    def fill_in_other_fields(self):
        self.less_discount_total = SalesItem.objects.filter(
            sales=self).aggregate(val=Sum('less_discount'))['val'] or 0
        self.less_vat_total = SalesItem.objects.filter(
            sales=self).aggregate(val=Sum('less_vat'))['val'] or 0
        self.total = SalesItem.objects.filter(sales=self).aggregate(
            val=Sum(F('unit_price') * F('quantity')))['val'] or 0
        self.with_discount_total = SalesItem.objects.filter(sales=self, less_discount__gt=0).aggregate(
            val=Sum(F('unit_price') * F('quantity')))['val'] or 0
        ...
        self.item_count = SalesItem.objects.filter(
            sales=self).aggregate(val=Sum('quantity'))['val'] or 0
        self.vatable = SalesItem.objects.filter(
            sales=self, vatable__isnull=False).aggregate(val=Sum('vatable'))['val'] or 0
        self.vat = SalesItem.objects.filter(
            sales=self, vat_amount__isnull=False).aggregate(val=Sum('vat_amount'))['val'] or 0
        self.zero_rated = SalesItem.objects.filter(
            sales=self, zero_rated__isnull=False).aggregate(val=Sum('zero_rated'))['val'] or 0
        self.vat_exempt = SalesItem.objects.filter(
            sales=self, vat_exempt__isnull=False).aggregate(val=Sum('vat_exempt'))['val'] or 0
        self.payable = decimal.Decimal(
            self.total) - self.less_discount_total - self.less_vat_total
        self.tendered = SalesPayment.objects.filter(
            sales=self).aggregate(val=Sum('amount'))['val'] or 0
        print(self.tendered)
    ...
    def price_composition(self, product, subtotal):
        ...
        if product.tax_type == 'V':  # VATable
            ...
            zero_rated = subtotal
        print(f'get_vatable_percentage(): {get_vatable_percentage()}')
        print(vatable, vat_amount, vat_exempt, zero_rated)
    ...
    def add_product(self, barcode, quantity):
        ...
        if product:
            ...
            if product.status != 'ACTIVE':
                ...
                item = SalesItem.objects.filter(
                    sales=self, product=product, is_wholesale=True)
                if item:
                    first = item.first()
                    # get qty
                ...
                item = SalesItem.objects.filter(
                    sales=self, product=product, is_wholesale=False)
                if item:
                    first = item.first()
                ...
                ws_qty = int(
                    qty / product.wholesale_qty) if product.wholesale_qty > 0 else 0
                qty = qty - ws_qty * product.wholesale_qty
                if ws_qty > 0:
                    subtotal = ws_qty * product.wholesale_price
                    vatable, vat_amount, vat_exempt, zero_rated = self.price_composition(
                        product, subtotal)
    ...
    def complete(self, cashier, details=None):
        ...
        if items:
            for item in items:
                cogs = item.product.purchase(
                    item.quantity, item.is_wholesale, cashier)
...
class SalesItem(models.Model):
    ...
    def fill_in_other_fields(self):
        ...
        if self.product.tax_type == 'V':
            self.vatable = self.subtotal
            self.vat_amount = self.subtotal * \
                decimal.Decimal(get_vatable_percentage() / 100)
    ...
    def apply_discount(self, discount_type, count=0):
        ...
        if self.product.tax_type == 'V' \
            ...
            # the product will become VAT-Exempt less discount
            self.vat_exempt = self.vatable - discount_amt
            self.vatable = 0
...
class SalesInvoice(models.Model):
    ...
    def fill_in_other_fields(self):
        self.is_cancelled = SalesVoid.objects.filter(
            sales_invoice=self).exists()
